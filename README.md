...
`supabase/migrations/`에 순서대로 번호가 매겨진 SQL 파일이 있습니다(`0001`~`0006`). 아래 중 한 가지 방법으로 적용합니다.
...
1. 새 Supabase 프로젝트를 만들고 `supabase/migrations/`의 `0001`~`0006`를 그 프로젝트에 적용
...
동료 초대·약속, 방문 확인·리뷰, 먹은 메뉴 기록, 사진, 상태 제보, 즐겨찾기, 도감, 내부 알림, 신고, CSV 업로드, 관리자 사용자·식당 활성화 관리 화면, Kakao Map 지도 임베드.
