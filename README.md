...
`supabase/migrations/`에 순서대로 번호가 매겨진 SQL 파일이 있습니다(`0001`~`0012`). 아래 중 한 가지 방법으로 적용합니다.
...
`0011_admin_atomic_operations.sql`은 직원 PIN·활성 상태, 신고 처리, CSV batch 반영과 관리자 감사 로그를 각각 하나의 트랜잭션으로 처리하는 서버 전용 RPC를 추가합니다. 함수 실행 권한은 `service_role`에만 부여됩니다.

`0012_meal_records.sql`은 완료된 개인 방문·약속별 대표 메뉴명과 실제 지불 가격을 스냅샷으로 저장합니다.
...
1. 새 Supabase 프로젝트를 만들고 `supabase/migrations/`의 `0001`~`0012`를 그 프로젝트에 적용
...
- "여기로 결정"과 개인 방문(`visits` 테이블): 추천 결과·식당 상세에서 결정, 홈 화면에서 오늘의 점심 확인, 다른 식당으로 변경, 결정 취소, 방문 완료. 완료된 방문이 있으면 같은 날 추가 결정·변경·취소 불가. 완료된 방문은 추천 엔진의 최근 방문 감점(최근 14일, 가중치 0.2)에 반영되며, 조건 추천에서 "최근 방문 제외"로 완전히 뺄 수도 있음
- 동료 초대·약속(`appointments`/`appointment_participants` 테이블, 개인 방문과는 독립): "동료와 함께"로 약속 생성(기본 시각 자동 계산, 메모, 닉네임 직접 초대), 초대 링크로 자율 참여(수락/거절), 참여 확정 후 불참, 방장의 시각·메모·식당 변경과 약속 전체 취소, 홈 화면 "다가오는 약속" 표시. 비로그인 상태로 초대 링크를 열면 로그인/가입 후 원래 화면으로 복귀
- 방문 확인(다녀왔어요/가지 않았어요/나중에 할게요): 개인 방문은 결정 시각 1시간 후, 동료 약속은 예정 시각 1시간 후부터 홈 화면 "방문 확인" 섹션에 우선 노출. 서버에서도 조기 확인을 차단하며 참여자별로 개별 확인하고, 방장 확인은 참여자 테이블과 분리된 컬럼으로 관리(`appointments.host_attendance_status`)
- 실제 먹은 메뉴·가격(`meal_records` 테이블): 완료된 개인 방문 또는 약속마다 등록 메뉴 선택이나 직접 입력으로 대표 메뉴 1개와 실제 지불 가격 저장. 같은 완료 기록은 재저장 시 수정되며 메뉴명·가격은 스냅샷으로 보존하고 홈·약속 상세·도감에 표시
- 모바일 하단 내비게이션과 내 정보: 로그인 직원 화면에서 홈·식당 찾기·도감·내 정보로 이동하고, `/me`에서 가입일과 완료 방문·약속 참여·리뷰·즐겨찾기 수, 알림 링크, 로그아웃 제공
- 월간 배지·리더보드(`/leaderboard`): Asia/Seoul 월 기준 리뷰왕·점심 개척왕·메뉴 수집왕의 공동 순위, 상위 직원과 내 순위 표시
- 리뷰(`reviews` 테이블): 방문 완료 이력이 있는 식당만 작성 가능, 직원당 식당별 1개(재작성 시 수정). 필수 4종(맛/속도/가격만족도/혼밥적합성) + 재방문 의향 + 선택 항목(양/혼잡/단체적합성/청결/태그/한 줄 후기). 식당 상세에 평균 평점·최근 후기 표시, `/recommend`·`/restaurants`에 리뷰 수·리뷰 많은순 정렬 반영. 개인 방문과 약속 참여 완료 모두 추천 엔진의 최근 방문 감점에 함께 반영
...
관리자 기능은 UUID·설정값·CSV 파일·저장 batch·변경 이력 snapshot을 서버에서 검증하고, DB 오류와 대상 없음 및 감사 로그 저장 실패를 구분합니다. 예상 가능한 결과는 각 관리자 화면에서 상태 메시지로 안내합니다. CSV 적용, 신고 처리, 직원 보안 작업은 `0011`의 서버 전용 RPC가 데이터 변경과 감사 로그를 하나의 트랜잭션으로 처리합니다.

## 관리자 CSV 입력

메뉴와 영업시간 CSV는 Kakao 식당 ID로 연결하며, 템플릿 다운로드·업로드 미리보기·행별 오류 안내를 제공합니다. 오류가 하나라도 있으면 적용을 차단하고, 정상 배치는 `0011` RPC가 데이터 변경과 관리자 로그를 한 트랜잭션으로 처리합니다. 브레이크타임은 현재 화면과 추천에서 사용하지 않아 CSV 입력 대상에서 제외했습니다.
