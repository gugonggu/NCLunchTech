...
- 방문 확인(다녀왔어요/가지 않았어요/나중에 할게요): 개인 방문은 결정 직후, 동료 약속은 예정 시각부터 홈 화면 "방문 확인" 섹션에 우선 노출. 미래 약속은 서버에서도 조기 확인을 차단하며 참여자별로 개별 확인하고, 방장 확인은 참여자 테이블과 분리된 컬럼으로 관리(`appointments.host_attendance_status`)
