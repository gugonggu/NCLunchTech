...
`supabase/migrations/`에 순서대로 번호가 매겨진 SQL 파일이 있습니다(`0001`~`0008`). 아래 중 한 가지 방법으로 적용합니다.
...
1. 새 Supabase 프로젝트를 만들고 `supabase/migrations/`의 `0001`~`0008`를 그 프로젝트에 적용
...
- 방문 확인(다녀왔어요/가지 않았어요/나중에 할게요): 개인 방문은 결정 후, 약속은 예정 시각 후 1시간이 지나면 홈 화면 "방문 확인" 섹션에 우선 노출. 참여자별로 개별 확인하며 방장 확인은 참여자 테이블과 분리된 컬럼으로 관리(`appointments.host_attendance_status`)
...
먹은 메뉴 실제 가격 개인 기록(현재는 기존 공동 편집 메뉴 화면 재사용으로 대체), 사진, 상태 제보, 즐겨찾기, 도감, 내부 알림, 신고, CSV 업로드, 관리자 사용자·식당 활성화 관리 화면, Kakao Map 지도 임베드.
