...
- Supabase(PostgREST) 기본 조회 행 수 제한(1,000건)에 걸려, 전체 활성 식당(1,312건)을 한 번에 조회하는 `/restaurants`, `/recommend`, `/collection`이 모두 실제로는 1,000건까지만 조회합니다. `.range()`/`.limit()`로 명시적으로 늘려야 하며 아직 반영되지 않았습니다.
