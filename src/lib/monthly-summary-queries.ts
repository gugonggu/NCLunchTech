...
export async function getMonthlySummary(employeeId: string, now = new Date()) {
  ...
  const [{ data: visits }, { data: hosted }, { data: participants }, { data: reviews }, { data: meals }, restaurants] = await Promise.all([
    ...
    fetchAllRows((from, to) => supabase.from("restaurants").select("id, name").eq("is_active", true).range(from, to)),
  ]);
