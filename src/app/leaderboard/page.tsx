  ...
  return (
    <main className="flex flex-1 flex-col gap-5 bg-brand-bg px-6 py-8">
      <div>
        ...
        <p className="mt-1 text-sm text-neutral-500">집계 기간 · {leaderboard.label}</p>
      </div>
      ...
      {(Object.keys(CATEGORY_META) as Array<keyof typeof CATEGORY_META>).map((key) => {
        ...
        return (
          <section key={key} className="rounded-2xl bg-white px-4 py-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="shrink-0 rounded-full bg-orange-50 px-3 py-1 text-xs font-semibold text-brand">
                월간 배지
              </span>
            </div>

            {category.leaders.length === 0 ? (
              <p className="mt-4 rounded-xl bg-neutral-50 px-3 py-3 text-sm text-neutral-500">
                이번 달 기록이 아직 없어요.
              </p>
            ) : (
              <ol className="mt-4 flex flex-col gap-2">
                {category.leaders.map((row) => (
                  <li key={row.employeeId} className="flex items-center justify-between rounded-xl bg-orange-50 px-3 py-2">
            ...
            <p className="mt-3 border-t border-neutral-100 pt-3 text-sm font-semibold text-brand-dark">
              {category.myRank
