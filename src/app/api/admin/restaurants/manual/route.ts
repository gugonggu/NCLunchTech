...
export async function POST(request: Request) {
  ...
  if (!b) return NextResponse.json({ message: "입력값을 다시 확인해주세요." }, { status: 400 });
  ...
  if (error?.code === "23505") {
    const { data: concurrentlyCreated } = await s.from("restaurants").select("id").eq("kakao_place_id", id).maybeSingle();
    if (concurrentlyCreated) return NextResponse.json({ id: concurrentlyCreated.id, existing: true });
  }
