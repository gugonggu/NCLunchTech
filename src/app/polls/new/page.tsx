...
export default async function NewPollPage({
      ...
      <main className="flex flex-1 flex-col gap-4 px-6 py-8">
        ...
        <div className="flex flex-col gap-3">
          <Link
            ...
            className="rounded-2xl bg-brand px-4 py-4 text-center font-semibold text-white"
          >
            식당 투표
          </Link>
          <Link
            href="/polls/new?type=menu"
            className="rounded-2xl bg-white px-4 py-4 text-center font-semibold text-brand-dark shadow-sm"
          >
            메뉴 투표
          </Link>
        </div>
  ...
  if (type === "restaurant") {
    ...
    return (
      <main className="flex flex-1 flex-col gap-4 px-6 py-8">
        <Link href="/polls/new" className="text-sm text-neutral-500">
          ← 뒤로
        </Link>
        ...
        <form action={createRestaurantPoll} className="flex flex-col gap-3">
          <p className="text-sm text-neutral-500">활성 식당 중 최대 {MAX_POLL_OPTIONS}개까지 고를 수 있어요.</p>
  ...
  if (!restaurantId) {
    ...
    return (
      <main className="flex flex-1 flex-col gap-4 px-6 py-8">
        ...
        {feedbackMessage && <p className="text-sm text-red-600">{feedbackMessage}</p>}
        <p className="text-sm text-neutral-600">먼저 메뉴를 고를 식당을 선택해주세요.</p>
  ...
  return (
    <main className="flex flex-1 flex-col gap-4 px-6 py-8">
      <Link href="/polls/new?type=menu" className="text-sm text-neutral-500">
        ← 다른 식당 고르기
      </Link>
      <h1 className="text-xl font-bold text-brand-dark">메뉴 투표 만들기</h1>
